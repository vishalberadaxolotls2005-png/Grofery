...
class NestedCategoryBloc
    extends Bloc<NestedCategoryEvent, NestedCategoryState> {
  ...
  Future<void> _onFetchNestedCategory(
      FetchNestedCategory event, Emitter<NestedCategoryState> emit) async {
    ...
    try {
      ...
      // Use SubCategoryRepository with slug parameter (correct API)
      ...
      // Map the response data to CategoryData objects
      subCategoryData = List<CategoryData>.from(
          response['data']['data'].map((data) => CategoryData.fromJson(data)));
      currentPage += 1;

      if (response['success'] == true) {
        emit(NestedCategoryLoaded(
            message: response['message'], subCategoryData: subCategoryData));
